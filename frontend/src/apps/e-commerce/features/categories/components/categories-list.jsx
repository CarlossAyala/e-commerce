    ...
    <div key={category.id} className="space-y-4">
      <Link to={categoryActionRoutes.details(category.id)}>
        <div className="relative">
          ...
          <div className="absolute inset-0 overflow-hidden rounded-md bg-gradient-to-t from-primary"></div>
          <div className="absolute inset-x-2 bottom-2">
            <h3 className="line-clamp-1 font-semibold uppercase leading-4 text-white tablet:text-xl">
              {category.name}
            </h3>
          </div>
        </div>
